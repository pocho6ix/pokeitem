/**
 * Applique les cardmarketUrl proposés par scripts/dry-run-neo-cm-urls.ts
 * pour les 4 séries Neo + Promos Wizards.
 *
 * Invariant exploité : avant l'apply, TOUTES les cartes de ces 5 séries
 * ont `cardmarketUrl = null` en DB (confirmé au moment du dry-run). Le
 * drift check refuse donc d'écraser toute valeur non-null.
 *
 * Workflow :
 *   1. Lit .dry-run-neo-cm-urls.json (doit avoir été regénéré juste avant)
 ...
 *      backups/neo-cm-urls-<ISODate>.json (pour rollback symétrique).
 ...
 *
 * Usage :
 *   npx tsx scripts/dry-run-neo-cm-urls.ts       # toujours refaire d'abord
 *   npx tsx scripts/apply-neo-cm-urls.ts         # dry check, montre le plan
 *   npx tsx scripts/apply-neo-cm-urls.ts --yes   # exécute
 */
...
const DRY_RUN_FILE = ".dry-run-neo-cm-urls.json";
...
async function main() {
  ...
  if (!existsSync(DRY_RUN_FILE)) {
    ...
    console.error(`   npx tsx scripts/dry-run-neo-cm-urls.ts`);
    process.exit(1);
  }
  ...
  if (drift.length) {
    ...
    console.error(`\nRégénère le dry-run : npx tsx scripts/dry-run-neo-cm-urls.ts`);
    process.exit(1);
  }
  ...
  const backupPath = join(BACKUP_DIR, `neo-cm-urls-${stamp}.json`);
  writeFileSync(
    backupPath,
