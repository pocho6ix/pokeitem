...
function SortModal({
  ...
  sortBy: SortKey;
  sortOrder: SortOrder;
  onApply: (by: SortKey, order: SortOrder) => void;
  onClose: () => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-sm rounded-2xl bg-[var(--bg-card)] p-6 shadow-xl">
          ...
          {SORT_OPTIONS.map((opt) => (
            <label key={opt.value} className="flex cursor-pointer items-center gap-3">
              <input
                ...
                name="sortBy"
                value={opt.value}
                checked={localBy === opt.value}
                onChange={() => setLocalBy(opt.value)}
        ...
        <p className="mb-3 text-sm font-medium text-[var(--text-secondary)]">Ordre de tri</p>
          ...
          {(["asc", "desc"] as const).map((o) => (
            <label key={o} className="flex cursor-pointer items-center gap-3">
              <input
                ...
                name="sortOrder"
                value={o}
                checked={localOrder === o}
                onChange={() => setLocalOrder(o)}
                className="accent-blue-500"
              />
        ...
        <div className="flex gap-3">
          <button
            onClick={() => { setLocalBy("number"); setLocalOrder("asc"); }}
          ...
          <button
            onClick={() => { onApply(localBy, localOrder); onClose(); }}
      ...
      <div className="w-full max-w-sm rounded-2xl bg-[var(--bg-card)] p-6 shadow-xl">
        ...
        <div className="space-y-2">
          <button
            ...
            className="flex w-full items-center gap-2 rounded-xl bg-red-50 px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-red-100 dark:bg-red-950/30 dark:text-red-400 dark:hover:bg-red-950/50"
          >
...
function AddToCollectionModal({
  ...
  onSubmit: (data: { quantity: number; condition: CardCondition; language: string; foil: boolean }) => void;
  onClose: () => void;
  isPending: boolean;
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 sm:items-center sm:p-4">
      <div className="w-full max-w-md rounded-t-2xl bg-[var(--bg-card)] p-6 shadow-xl sm:rounded-2xl">
          ...
          <div>
            ...
            <div className="flex items-center rounded-xl border border-[var(--border-default)] bg-[var(--bg-subtle)]">
              <button
                ...
                className="px-3 py-2 text-lg font-bold text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
              >−</button>
              <input
                type="number"
                min={1}
        ...
        <button
          onClick={() => onSubmit({ quantity, condition, language, foil })}
          disabled={isPending}
...
function AddToDoublesModal({
  ...
  selectedCards: CardRow[];
  onSubmit: (data: { quantity: number; condition: CardCondition; language: string; availability: DoubleAvailability; price: number | null }) => void;
  onClose: () => void;
  isPending: boolean;
}) {
  const [quantity,     setQuantity]     = useState(1);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 sm:items-center sm:p-4">
      <div className="w-full max-w-md rounded-t-2xl bg-[var(--bg-card)] p-6 shadow-xl sm:rounded-2xl">
        ...
        <div className="mb-5 rounded-xl border border-[var(--border-default)] p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                value={availability}
                onChange={(e) => setAvailability(e.target.value as DoubleAvailability)}
                className="w-full rounded-xl border border-[var(--border-default)] bg-[var(--bg-subtle)] px-3 py-2.5 text-sm text-[var(--text-primary)] outline-none focus:border-blue-500"
              >
        ...
        <button
          onClick={() => onSubmit({ quantity, condition, language, availability, price: price ? parseFloat(price) : null })}
          disabled={isPending}
          className="w-full rounded-xl bg-blue-600 py-3 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60"
        >
...
export function CardCollectionGrid({
  ...
  // Local mutable copy of owned/doubles data (for optimistic updates)
  const [ownedMap, setOwnedMap] = useState<Map<string, OwnedEntry>>(() =>
    new Map(initialOwned.map((e) => [e.cardId, e]))
  );
  const [doublesMap, setDoublesMap] = useState<Map<string, DoubleEntry>>(() =>
    new Map(initialDoubles.map((e) => [e.cardId, e]))
  );
  ...
  }, [cards, search, rarityFilter, viewFilter, sortBy, sortOrder, ownedMap]);
  ...
  const handleAddToCollection = useCallback(
    (data: { quantity: number; condition: CardCondition; language: string; foil: boolean }) => {
      if (!isAuthenticated) return;

      // Optimistic update
      const optimisticOwned = new Map(ownedMap);
      for (const card of selectedCards) {
        optimisticOwned.set(card.id, {
          id: `temp-${card.id}`,
          cardId: card.id,
      ...
      startTransition(async () => {
        try {
          const res = await fetch("/api/cards/collection", {
            ...
            body: JSON.stringify({
              cards: selectedCards.map((c) => ({ cardId: c.id, ...data })),
            }),
          });
          if (!res.ok) {
            // Revert on error
            setOwnedMap(new Map(initialOwned.map((e) => [e.cardId, e])));
            ...
            setOwnedMap((prev) => {
              ...
              for (const r of results) {
                if (r.record) m.set(r.cardId, { id: r.record.id, cardId: r.cardId, ...data });
              }
              return m;
            });
          }
  ...
  const handleAddToDoubles = useCallback(
    (data: { quantity: number; condition: CardCondition; language: string; availability: DoubleAvailability; price: number | null }) => {
      if (!isAuthenticated) return;

      // Optimistic update
      const optimisticDoubles = new Map(doublesMap);
      for (const card of selectedCards) {
        optimisticDoubles.set(card.id, {
          id: `temp-${card.id}`,
          cardId: card.id,
          ...data,
        });
      }
      setDoublesMap(optimisticDoubles);
      setActiveModal(null);
      setSelectedIds(new Set());
      ...
      startTransition(async () => {
        try {
          ...
          if (!res.ok) {
            setDoublesMap(new Map(initialDoubles.map((e) => [e.cardId, e])));
            alert("Erreur lors de l'ajout aux doubles. Veuillez réessayer.");
          } else {
            const { results } = await res.json();
            setDoublesMap((prev) => {
              const m = new Map(prev);
  ...
  const handleRemoveSelected = useCallback(() => {
    ...
    const optimistic = new Map(ownedMap);
  ...
  const handleAddAllToCollection = useCallback(() => {
    ...
    for (const c of missing) {
      optimistic.set(c.id, { id: `temp-${c.id}`, cardId: c.id, quantity: 1, condition: CardCondition.NEAR_MINT, language: "FR", foil: false });
    }
    setOwnedMap(optimistic);
    ...
    startTransition(async () => {
      try {
        const res = await fetch("/api/cards/collection", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            cards: missing.map((c) => ({ cardId: c.id, quantity: 1, condition: "NEAR_MINT", language: "FR", foil: false })),
          }),
        });
        if (!res.ok) setOwnedMap(new Map(initialOwned.map((e) => [e.cardId, e])));
      } catch {
        setOwnedMap(new Map(initialOwned.map((e) => [e.cardId, e])));
      }
    });
    ...
    startTransition(async () => {
      try {
        const res = await fetch("/api/cards/collection", {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ cardIds: ownedIds }),
        });
        if (!res.ok) setOwnedMap(new Map(initialOwned.map((e) => [e.cardId, e])));
      } catch {
        setOwnedMap(new Map(initialOwned.map((e) => [e.cardId, e])));
    ...
    <div className="relative pb-24">
      ...
      {rarityCounts.size > 0 && (
        <div className="mb-4 flex flex-wrap gap-1.5">
          {(Object.values(CardRarity) as CardRarity[])
        ...
        <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8">
          {filteredCards.map((card) => {
            ...
            return (
              <div
                key={card.id}
                onClick={() => isAuthenticated && toggleSelect(card.id)}
                className={cn(
                  "group relative",
              ...
              >
                ...
                >
                  ...
                  {/* Double badge */}
                  {hasDouble && (
                    <div className="absolute left-1 top-1 flex h-5 min-w-[20px] items-center justify-center rounded-full bg-amber-500 px-1 text-[9px] font-bold text-white shadow">
                      D
                    </div>
                  )}
      ...
      {isAuthenticated && (
        <div
          className={cn(
            "fixed bottom-16 left-0 right-0 z-40 transition-all duration-200 sm:bottom-0",
        ...
        >
          <div className="mx-auto max-w-7xl px-4 pb-4 sm:pb-6">
            <div className="flex items-center gap-3 rounded-2xl bg-[var(--bg-card)] p-3 shadow-2xl border border-[var(--border-default)]">
              ...
              {/* Add to doubles */}
              <button
                onClick={() => setActiveModal("add-doubles")}
              ...
              {/* Add to collection */}
              <button
                onClick={() => setActiveModal("add-collection")}
