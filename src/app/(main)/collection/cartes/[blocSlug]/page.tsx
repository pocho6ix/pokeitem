...
export async function generateMetadata({ params }: BlocPageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: "website",
      url: `https://app.pokeitem.fr/collection/cartes/${blocSlug}`,
    },
    twitter: { card: "summary_large_image", title, description },
    alternates: {
      canonical: `https://app.pokeitem.fr/collection/cartes/${blocSlug}`,
    },
  };
}

export default async function BlocCartesPage({ params }: BlocPageProps) {
  const { blocSlug } = await params;
  const bloc = findBloc(blocSlug);
  ...
  const itemListLd = generateItemListJsonLd(
    `Extensions de cartes — ${bloc.name}`,
    series.map((s) => ({
      name: s.name,
      url: `/collection/cartes/${bloc.slug}/${s.slug}`,
    }))
  );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <nav className="mb-6 flex items-center gap-2 text-sm text-[var(--text-secondary)]">
        <Link href="/collection/cartes" className="hover:text-blue-600 transition-colors">
          Collection
        </Link>
        <span>/</span>
      ...
      {series.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {series.map((serie) => (
            <Link
              key={serie.slug}
              href={`/collection/cartes/${bloc.slug}/${serie.slug}`}
            >
